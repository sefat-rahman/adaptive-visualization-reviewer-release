"""Full-point country FPS topology precompute for the OpenLayers app."""
...
class CountryFPSTopologyCache:
    """Precompute full-point topology for country-level FPS samples."""

    CACHE_VERSION = "country_fps_topology_full_v5"
    CACHE_DIRNAME = "country_fps_topology_cache"
    BASELINE_PERCENTAGES = (1, 2, 3, 4, 5)
    BASELINE_PERCENTAGE = 1
    PERCENTAGES = tuple(range(1, 51)) + tuple(range(55, 101, 5))
    ...
    def get_or_create(
        ...
        *,
        ...
        percentage = int(selection["percentage"])
        frame_key = f"country::{percentage}"
    ...
    def get_selection(
        ...
        *,
        ...
        curve = self._load_available_curve()
        if curve.empty or not self._curve_has_rows(curve, {requested_baseline}):
            curve = self.ensure_precomputed(valid_df)
    ...
    def get_payload(
        ...
        *,
        ...
        requested_baseline = self._requested_baseline_percentage(baseline_percentage)
        baseline_percentage = requested_baseline
        current_percentage = int(max(baseline_percentage, min(100, current_percentage)))
        curve = self._load_available_curve()
        if curve.empty or not self._curve_has_rows(curve, {baseline_percentage}):
            curve = self.ensure_precomputed(source_df)
        if curve.empty:
            return {
                "original": {"h0": [], "h1": []},
                "reduced": {"h0": [], "h1": []},
        ...
        baseline_row = self._baseline_row(curve, baseline_percentage)
        if baseline_row is None:
            baseline_row = curve.iloc[0]
        baseline_percentage = int(baseline_row["percentage"])
        snapped_percentage = self._available_percentage_at_or_below(curve, current_percentage)
        if snapped_percentage is None:
            snapped_percentage = int(curve.iloc[-1]["percentage"])
        current_percentage = snapped_percentage
        ...
        base_dir = self.cache_dir / "country"
    ...
    def ensure_precomputed(self, source_df: pd.DataFrame) -> pd.DataFrame:
        return self.ensure_precomputed_until(source_df)
    ...
    def ensure_precomputed_until(
        ...
        curve_path = self.cache_dir / "country" / "curve.csv"
        if (
            self._curve is not None
            and self._metadata_matches_signature(signature)
            and self._curve_complete(self._curve, max_percentage=max_percentage)
        ):
            return self._curve

        metadata = self._load_metadata()
        if curve_path.exists() and self._is_valid(metadata, signature, curve_path):
            curve_df = pd.read_csv(curve_path)
            if self._curve_complete(curve_df, max_percentage=max_percentage):
                self._curve = curve_df
                return curve_df

        curve_df = self._compute_curve(valid_df, signature, max_percentage=max_percentage)
        self._curve = curve_df
        return curve_df
    ...
    def _compute_curve(
        ...
        fps_order = self.fps_order_cache.get_order(
            zoom="country",
            state="",
            county="",
            source_df=source_df,
        ...
        rows_by_percentage = self._load_existing_rows(curve_path, partial_curve_path)
        ...
        metadata = self._load_metadata()
        metadata.setdefault("entries", {})["country"] = {
    ...
    def _distance_block(self, original: dict[str, np.ndarray], reduced: dict[str, np.ndarray]) -> dict[str, float]:
        return {
            "bottleneck_h0": self._safe_distance(original["h0"], reduced["h0"], bottleneck_distance),
            "bottleneck_h1": self._safe_distance(original["h1"], reduced["h1"], bottleneck_distance),
            "wasserstein_h0": self._safe_wasserstein(original["h0"], reduced["h0"]),
    ...
    @staticmethod
    def _metric_block(row: pd.Series) -> dict[str, float]:
        return {metric: float(row[metric]) for metric in CountryFPSTopologyCache.METRICS}
    ...
    def _metadata_matches_signature(self, signature: dict[str, Any]) -> bool:
        metadata = self._load_metadata()
        info = metadata.get("entries", {}).get("country")
        return isinstance(info, dict) and info.get("source_signature") == signature

    def _is_valid(self, metadata: dict[str, Any], signature: dict[str, Any], curve_path: Path) -> bool:
    ...
    def _curve_complete(self, curve_df: pd.DataFrame, max_percentage: int | None = None) -> bool:
        ...
        if max_percentage is not None:
            max_percentage = int(max(self.BASELINE_PERCENTAGE, min(100, max_percentage)))
    ...
    def _configured_percentages(cls, max_percentage: int | None = None) -> tuple[int, ...]:
        ...
        max_percentage = int(max(cls.BASELINE_PERCENTAGE, min(100, max_percentage)))
    ...
    def _load_available_curve(self) -> pd.DataFrame:
        base_dir = self.cache_dir / "country"
    ...
    def _select_curve_row(
        ...
        *,
        ...
        match = ordered[
            ordered.apply(
                lambda row: cls._normalized_metric_value(row, metric_name, baseline_row) <= threshold,
                axis=1,
            )
        ]
        if not match.empty:
    ...
    @staticmethod
    def _baseline_row(curve_df: pd.DataFrame, baseline_percentage: int) -> pd.Series | None:
        if curve_df.empty:
            return None
    ...
    @staticmethod
    def _available_percentage_at_or_below(curve_df: pd.DataFrame, percentage: int) -> int | None:
